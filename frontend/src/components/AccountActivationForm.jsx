...
const AccountActivationForm = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      await api.post('/auth/send-otp', { email, purpose: 'activation' });
      setStep(2);
    } catch (err) {
      ...
      if (err.response?.status === 429) {
        ...
        setError("Account already activated. Use 'Forgot Password' to reset your password.");
      } else {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/verify-otp-set-password', { email, otp, password, purpose: 'activation' });
      setSuccess(true);
    } catch (err) {
      const code = err.response?.data?.error?.code;
