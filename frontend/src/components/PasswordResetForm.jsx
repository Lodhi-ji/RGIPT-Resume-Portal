...
const PasswordResetForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (err.response?.data?.error?.code === 'NOT_ACTIVATED') {
        setError('Account not activated. Please use \'Sign Up\' first.');
      } else {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Enter your institute email address and we'll send you a new password.
          </p>
        </div>
